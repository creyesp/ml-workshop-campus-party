"""Session-level features v3 vs. v2 + architecture sweep (spec iteration 2).

1. Leakage check on the new session features (directed AUC; flag >= 0.85).
2. Architecture x feature-set matrix: CV PR-AUC + CI for each model on v2 (22
   feat) vs v3 (32 feat), same train rows.
3. Tune XGBoost on v3 and compare its CI to the v2 champion (0.471 [0.446,0.497]).

Capped parallelism: estimators single-threaded; Optuna trials at N_JOBS.

Run: .venv/bin/python experiments/10_features_v3.py [n_trials]
"""
import json
import sys
...
V2_CHAMP_LO, V2_CHAMP_HI = 0.446, 0.497  # tuned XGBoost on v2 (task 17)
...
def main():
    ...
    print("=== leakage check on new session features (directed AUC) ===")
    leak = pd.DataFrame(
    ...
    print(f"{'model':10s} {'v2 (22 feat)':>22s} {'v3 (32 feat)':>22s}   Δ")
    ...
    print(f"\n=== tuning XGBoost on v3 features ({n_trials} trials) ===")
    ...
    print(f"tuned XGBoost v3  PR-AUC={pt:.3f}  CI=[{lo:.3f}, {hi:.3f}]")
    print(f"v2 champion       PR-AUC=0.471  CI=[{V2_CHAMP_LO}, {V2_CHAMP_HI}]")
    improved = lo > V2_CHAMP_HI
    print("\nVERDICT: " + (
        f"v3 IMPROVES — CI [{lo:.3f},{hi:.3f}] above v2 champion CI [{V2_CHAMP_LO},{V2_CHAMP_HI}] (non-overlapping)."
        if improved else
        f"v3 does NOT beat v2 with non-overlapping CIs (CI [{lo:.3f},{hi:.3f}] vs [{V2_CHAMP_LO},{V2_CHAMP_HI}])."))

    with start_run("v3-xgb-tuned"):
        ...
        mlflow.set_tag("v3_improves", str(improved))
    json.dump(best, open("experiments/_artifacts/xgb_v3_best_params.json", "w"), indent=2)
