"""Difficulty/progression features v6 vs. v3 + architecture sweep (iteration 4).

1. Leakage check on the new difficulty features (directed AUC; flag >= 0.85).
2. Architecture x feature-set matrix: CV PR-AUC + CI for each model on v2 (22
   feat) vs v3 (32 feat), same train rows.
3. Tune XGBoost on v3 and compare its CI to the v3 champion (0.471 [0.446,0.497]).
...
V3 = config.V6_NUMERICAL_COLUMNS
...
def main():
    ...
    train = load_v6("train", verify=True)
    y = train[config.LABEL_COLUMN].values

    new_feats = [c for c in V3 if c not in V2]
    print("=== leakage check on new difficulty features (directed AUC) ===")
    leak = pd.DataFrame(
    ...
    print(f"{'model':10s} {'v3 (32 feat)':>22s} {'v6 (43 feat)':>22s}   Δ")
    ...
    print(f"\n=== tuning XGBoost on v6 features ({n_trials} trials) ===")
    ...
    print(f"tuned XGBoost v6  PR-AUC={pt:.3f}  CI=[{lo:.3f}, {hi:.3f}]")
    print(f"v3 champion       PR-AUC=0.548  CI=[{V2_CHAMP_LO}, {V2_CHAMP_HI}]")
    improved = lo > V2_CHAMP_HI
    print("\nVERDICT: " + (
        f"v6 IMPROVES — CI [{lo:.3f},{hi:.3f}] above v3 champion CI [{V2_CHAMP_LO},{V2_CHAMP_HI}] (non-overlapping)."
        if improved else
        f"v6 does NOT beat v2 with non-overlapping CIs (CI [{lo:.3f},{hi:.3f}] vs [{V2_CHAMP_LO},{V2_CHAMP_HI}])."))

    with start_run("v6-xgb-tuned"):
        mlflow.log_params(best)
        mlflow.log_param("feature_set", "v6")
        mlflow.log_metric("pr_auc", pt)
        mlflow.log_metric("pr_auc_ci_low", lo)
        mlflow.log_metric("pr_auc_ci_high", hi)
        mlflow.set_tag("v6_improves", str(improved))
    json.dump(best, open("experiments/_artifacts/xgb_v6_best_params.json", "w"), indent=2)
